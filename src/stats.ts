export function wordsCounter() {
    return function (input: string, prevCh?: string, subseqCh?: string): number { 
        ...
        // Create patterns that match the required counting behavior 
        const pattern = new RegExp(
            [
              `(?:[0-9]+(?:(?:,|\\.)[0-9]+)*|[\\-${spaceDelimitedChars}0-9])+`, //add 0-9 into the later pattern to match abc123 as one word
...
//console.log(`Input words: ${(input.match(pattern) || []).length}`)
        return (afterInput.match(pattern) || []).length - (beforeInput.match(pattern)||[]).length;
  };
}
